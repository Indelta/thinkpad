            ...
            $.ajax({
                ...
                success: function () {
                    if (form.hasClass('slider')){
                        yaCounter45860163.reachGoal('T470sGetConsultation');
                        ga('send', 'event', 'Btn', 'Click', 'T470sGetConsultation');
                    } else if (form.hasClass('calculator')){
                        yaCounter45860163.reachGoal('T470sCalculatorConsultation');
                        ga('send', 'event', 'Btn', 'Click', 'T470sCalculatorConsultation');
                    }
                    fbq('track','Lead');
    ...
    $('.contactForm').submit(function () {
        ...
        if (email.val().length === "") {
            ...
            $.ajax({
                ...
                cache: false,
                success: function () {
                    yaCounter45860163.reachGoal('SendContactsLaw');
                    ga('send', 'event', 'Btn', 'Click', 'SendContactsLaw');
                    fbq('track','Lead');
                    form.fadeOut(300, function () {
                        form.trigger('reset');
                        $(this).after("<div class='thankYou'>Спасибо за Ваш запрос. Менеджер свяжется с Вами в ближайшее время</div>");
